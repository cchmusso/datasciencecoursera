...
we obtain the storm data in bzip archive format and extract it (if not aleady done)
...
Then we read in the extracted file, which is in CSV format, into a data frame. 
Our data is composed of 902297 rows and 37 variables. 
...
stormsByYear <- aggregate(EVTYPE~year, data=storm, FUN=length)
plot(stormsByYear, type='l', main="Nb events per year", ylab="nb events")
...
dim(storm)
```
Now our dataset is composed of only 681500 observations
...
p1 + scale_y_log10() +
  ...
  xlab("Type of Severe Weather Event")
p1 + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
